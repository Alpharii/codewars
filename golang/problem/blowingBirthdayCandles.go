...
func BlowCandles(str string) int {
    // parse string jadi slice of int
    candles := make([]int, len(str))
    for i, ch := range str {
        candles[i] = int(ch - '0')
    }
    ...
    for {
        // cari indeks pertama yang masih >0
        start := -1
        ...
        // sekali hembusan: kurangi hingga 3 lilin terdepan
        for j := start; j < start+3 && j < len(candles); j++ {
            if candles[j] > 0 {
                candles[j]--
            }
        }
